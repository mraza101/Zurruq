document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('a[href^="#"]').forEach(anchor => {
    anchor.addEventListener('click', function (e) {
      ...
      if (targetElement) {
        ...
        if (mobileMenu && mobileMenu.classList.contains('open')) mobileToggle.click();
      }
    });
  });
  ...
  if (contactForm && enquiryType) {
    ...
    if (typeParam === 'sample') enquiryType.value = 'sample';
    else if (typeParam === 'quote') enquiryType.value = 'quote';
    const updateFields = () => {
  ...
  dropdowns.forEach(dropdown => {
    ...
    if (trigger && menu) {
      ...
      trigger.addEventListener('click', (e) => { e.preventDefault(); toggleDropdown(); });
      trigger.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleDropdown(); }
        else if (e.key === 'Escape' && dropdown.classList.contains('focus-within')) { e.preventDefault(); toggleDropdown(false); trigger.focus(); }
        else if (e.key === 'ArrowDown' && dropdown.classList.contains('focus-within')) { e.preventDefault(); const firstLink = menu.querySelector('a'); if (firstLink) firstLink.focus(); }
      });
      menu.addEventListener('keydown', (e) => {
        const links = Array.from(menu.querySelectorAll('a'));
        const index = links.indexOf(document.activeElement);
        if (e.key === 'Escape') { e.preventDefault(); toggleDropdown(false); trigger.focus(); }
        else if (e.key === 'ArrowDown') { e.preventDefault(); if (index < links.length - 1) links[index + 1].focus(); }
        else if (e.key === 'ArrowUp') { e.preventDefault(); if (index > 0) links[index - 1].focus(); else trigger.focus(); }
      });
      document.addEventListener('click', (e) => { if (!dropdown.contains(e.target) && dropdown.classList.contains('focus-within')) toggleDropdown(false); });
      dropdown.addEventListener('focusout', (e) => { if (!dropdown.contains(e.relatedTarget)) toggleDropdown(false); });
    }
  });
